...
A common misconception with is that Windows does not lend itself well to configuration management tools, however products such as Chef and Puppet have mature Windows support with the [later having an extensive library of Windows modules for use.](https://forge.puppet.com/puppetlabs/windows)
...
Taking this approach will enasure that the SOE is free from credentials as well as portable across environments without the need for producing additional image updates which can be a timely exersize as your environment grows.

Another important When externalising these runtime values is to ensure you build resilience into the scripts that fetch them via to retry and backoff if they are not immediately available, or to gracefully handle transient issues in the environment.
